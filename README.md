...
- `charts/` — unarchived, patched chart working trees (output of `make prepare`/`make charts`) — gitignored on `main`, see [Chart releases](#chart-releases)
- `assets/` — versioned `.tgz` chart archives + `index.yaml` (the actual Helm repo output) — gitignored on `main`, see [Chart releases](#chart-releases)
...
- `.github/workflows/release-charts.yaml` — builds, publishes to GHCR and archives every package a merge to `main` touches (see [Chart releases](#chart-releases))
...
`main` — it's never part of a PR. CI regenerates and publishes it on merge;
see [Chart releases](#chart-releases). Running `make charts` locally is still
...
changed package runs `make prepare` + `make charts` and, for every archive
that produces:

1. pushes it to GHCR as an OCI artifact:
   ```
   oci://ghcr.io/therealgambo/k8s-charts/<chart-name>:<version>
   ```
2. commits the archive, its unarchived chart folder, and the updated
   `index.yaml` to the long-lived `charts` branch — this repo's actual
   distribution output. `charts/` and `assets/` are gitignored on `main`
   precisely because this branch is where they're kept instead.

Each package is released independently within the run, so a broken patch in
one package doesn't block releasing the others in the same push.
`workflow_dispatch` (or a push with no previous commit to diff against, e.g.
the first run after this workflow is added) rebuilds and republishes every
tracked package, since there's nothing to diff.
...
...or from the `charts` branch's `index.yaml` if it's served as a
traditional Helm HTTP repo (see `config/configuration.yaml`'s `helmRepo`
setting).
